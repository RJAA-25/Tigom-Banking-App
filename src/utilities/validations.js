...
export const checkPassword = (value, confirmation) => {
  ...
  return value !== confirmation ? "Confirmation password does not match" : "";
};
...
export const checkEmail = (value) => {
  ...
  const accounts = JSON.parse(localStorage.getItem("accounts")).users;
...
export const checkWithdraw = (value, balance) => {
...
export const checkExpenseName = (value) => {
  return value.trim().length === 0 ? "Expense name is required" : "";
};

export const checkExpenseAmount = (value) => {
  return Number(value) < 1 ? "Minimum allowed expense is ₱1.00" : "";
};
